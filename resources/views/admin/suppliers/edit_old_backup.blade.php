...
@section('title', 'Modifier ' . $supplier->company_name . ' - ZenFleet Enterprise')
...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 -m-6 p-6">
 ...
 <a href="{{ route('admin.suppliers.index') }}" class="hover:text-orange-600 transition-colors">
 Gestion des Fournisseurs
 </a>
 <i class="fas fa-chevron-right text-gray-400 text-xs"></i>
 <span class="font-semibold text-gray-900">Modifier {{ $supplier->company_name }}</span>
 </nav>

 <!-- Hero Content -->
 <div class="flex items-center justify-between">
 <div class="flex items-center gap-6">
 <div class="w-16 h-16 bg-gradient-to-br from-orange-600 to-red-600 rounded-2xl flex items-center justify-center shadow-lg">
 <i class="fas fa-edit text-white text-2xl"></i>
 </div>
 <div>
 <h1 class="text-4xl font-bold text-gray-900">Modifier le Fournisseur</h1>
 <p class="text-gray-600 text-lg mt-2">
 <span class="font-semibold text-orange-600">{{ $supplier->company_name }}</span>
 ...
 <!-- Supplier Info Card -->
 <div class="bg-orange-50 rounded-xl p-4 text-center min-w-[200px]">
 <div class="flex items-center justify-center gap-3 mb-2">
 <div class="w-12 h-12 rounded-full flex items-center justify-center text-white text-sm font-medium shadow-lg"
 style="background-color: {{ $supplier->rating_color }};">
 <i class="fas fa-star"></i>
 </div>
 <div>
 <div class="text-sm font-semibold text-gray-900">{{ $supplier->company_name }}</div>
 <div class="text-xs text-gray-500">Note: {{ number_format($supplier->rating ?? 5, 1) }}/10</div>
 ...
 <div x-ref="progressBar" class="h-full bg-gradient-to-r from-orange-500 to-red-500 transition-all duration-500 ease-out"></div>
 </div>
 </div>
 </div>
 </div>
 ...
 :class="currentStep >= 1 ? 'bg-orange-100 text-orange-600' : 'bg-gray-200 text-gray-400'">
 <i class="fas fa-building"></i>
 </div>
 <div class="hidden md:block">
 <div class="text-sm font-semibold" :class="currentStep >= 1 ? 'text-orange-600' : 'text-gray-400'">
 Informations Générales
 </div>
 </div>
 </div>

 <div class="w-8 h-0.5 bg-gray-300" :class="currentStep > 1 ? 'bg-orange-500' : 'bg-gray-300'"></div>

 <!-- Step 2 -->
 <div class="flex items-center gap-3">
 <div class="w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300"
 :class="currentStep >= 2 ? 'bg-orange-100 text-orange-600' : 'bg-gray-200 text-gray-400'">
 <i class="fas fa-user"></i>
 </div>
 <div class="hidden md:block">
 <div class="text-sm font-semibold" :class="currentStep >= 2 ? 'text-orange-600' : 'text-gray-400'">
 Contact Principal
 </div>
 </div>
 </div>

 <div class="w-8 h-0.5 bg-gray-300" :class="currentStep > 2 ? 'bg-orange-500' : 'bg-gray-300'"></div>

 <!-- Step 3 -->
 <div class="flex items-center gap-3">
 <div class="w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300"
 :class="currentStep >= 3 ? 'bg-orange-100 text-orange-600' : 'bg-gray-200 text-gray-400'">
 <i class="fas fa-map-marker-alt"></i>
 </div>
 <div class="hidden md:block">
 <div class="text-sm font-semibold" :class="currentStep >= 3 ? 'text-orange-600' : 'text-gray-400'">
 Localisation
 </div>
 </div>
 </div>

 <div class="w-8 h-0.5 bg-gray-300" :class="currentStep > 3 ? 'bg-orange-500' : 'bg-gray-300'"></div>

 <!-- Step 4 -->
 <div class="flex items-center gap-3">
 <div class="w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300"
 :class="currentStep >= 4 ? 'bg-orange-100 text-orange-600' : 'bg-gray-200 text-gray-400'">
 <i class="fas fa-cog"></i>
 </div>
 <div class="hidden md:block">
 <div class="text-sm font-semibold" :class="currentStep >= 4 ? 'text-orange-600' : 'text-gray-400'">
 Paramètres & Notes
 </div>
 </div>
 </div>
 ...
 <!-- Form Content -->
 <div class="p-8">
 <form id="supplierEditForm" method="POST" action="{{ route('admin.suppliers.update', $supplier) }}" class="space-y-8">
 @csrf
 @method('PUT')
 ...
 <p class="text-gray-600">Modifiez les informations de base du fournisseur</p>
 </div>
 ...
 class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:border-orange-400 focus:ring-4 focus:ring-orange-50 transition-all">
 @error('company_name')
 ...
 (Actuel: {{ $supplier->supplier_type_label }})
 </span>
 @endif
 </label>
 <select id="supplier_type" name="supplier_type" required
 class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:border-orange-400 focus:ring-4 focus:ring-orange-50 transition-all appearance-none">
 ...
 <input type="text" id="trade_register" name="trade_register" value="{{ old('trade_register', $supplier->trade_register) }}"
 ...
 <input type="text" id="nif" name="nif" value="{{ old('nif', $supplier->nif) }}"
 ...
 <p class="text-gray-600">Modifiez les informations de contact du fournisseur</p>
 </div>
 ...
 <input type="text" id="contact_first_name" name="contact_first_name" value="{{ old('contact_first_name', $supplier->contact_first_name) }}" required
 class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:border-orange-400 focus:ring-4 focus:ring-orange-50 transition-all">
 ...
 <input type="text" id="contact_last_name" name="contact_last_name" value="{{ old('contact_last_name', $supplier->contact_last_name) }}" required
 class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:border-orange-400 focus:ring-4 focus:ring-orange-50 transition-all">
 ...
 <input type="tel" id="contact_phone" name="contact_phone" value="{{ old('contact_phone', $supplier->contact_phone) }}" required
 class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:border-orange-400 focus:ring-4 focus:ring-orange-50 transition-all">
 @error('contact_phone')
 <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
 @enderror
 ...
 <input type="email" id="contact_email" name="contact_email" value="{{ old('contact_email', $supplier->contact_email) }}"
 class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:border-orange-400 focus:ring-4 focus:ring-orange-50 transition-all">
 @error('contact_email')
 <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
 @enderror
 ...
 <p class="text-gray-600">Modifiez l'adresse et la localisation du fournisseur</p>
 </div>
 ...
 class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:border-orange-400 focus:ring-4 focus:ring-orange-50 transition-all">{{ old('address', $supplier->address) }}</textarea>
 @error('address')
 <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
 @enderror
 </div>
 ...
 @if($supplier->wilaya)
 <span class="ml-2 text-xs text-gray-500">
 (Actuelle: {{ $supplier->wilaya_label }})
 </span>
 @endif
 </label>
 ...
 <option value="{{ $code }}" {{ old('wilaya', $supplier->wilaya) === $code ? 'selected' : '' }}>
 {{ $code }} - {{ $name }}
 </option>
 @endforeach
 </select>
 ...
 <input type="text" id="commune" name="commune" value="{{ old('commune', $supplier->commune) }}"
 class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:border-orange-400 focus:ring-4 focus:ring-orange-50 transition-all">
 @error('commune')
 <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
 @enderror
 ...
 <p class="text-gray-600">Modifiez la configuration et les informations complémentaires</p>
 </div>
 ...
 @if($supplier->rating)
 <span class="ml-2 text-xs text-gray-500">
 (Actuelle: {{ number_format($supplier->rating, 1) }})
 </span>
 @endif
 </label>
 <input type="number" id="rating" name="rating" value="{{ old('rating', $supplier->rating ?? 5) }}"
 ...
 <label class="flex items-center space-x-3">
 <input type="checkbox" name="is_certified" value="1" {{ old('is_certified', $supplier->is_certified) ? 'checked' : '' }}
 class="rounded border-gray-300 text-orange-600 focus:ring-orange-500">
 ...
 Modifier ces paramètres affectera l'évaluation et la priorité du fournisseur.
 @if($supplier->status_badge)
 Statut actuel: <strong>{{ $supplier->status_badge['label'] }}</strong>
 @endif
 </p>
 </div>
 </div>
 ...
 class="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl focus:border-orange-400 focus:ring-4 focus:ring-orange-50 transition-all">{{ old('notes', $supplier->notes) }}</textarea>
 @error('notes')
 <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
 @enderror
 </div>
 ...
 class="inline-flex items-center gap-3 px-6 py-3 bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 text-white rounded-xl font-semibold transition-all duration-200 shadow-sm hover:shadow-md">
 <span>Suivant</span>
 <i class="fas fa-arrow-right"></i>
 </button>

 <button type="submit" x-show="currentStep === 4"
 class="inline-flex items-center gap-3 px-8 py-3 bg-gradient-to-r from-green-600 to-orange-600 hover:from-green-700 hover:to-orange-700 text-white rounded-xl font-semibold transition-all duration-200 shadow-sm hover:shadow-md">
 ...
 <button type="submit" form="supplierEditForm"
 class="inline-flex items-center gap-3 px-6 py-4 bg-gradient-to-r from-green-600 to-orange-600 hover:from-green-700 hover:to-orange-700 text-white rounded-2xl font-semibold transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105">
