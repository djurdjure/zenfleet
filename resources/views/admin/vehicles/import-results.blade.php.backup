...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 -m-6 p-6">
 ...
 <p class="text-2xl font-bold text-amber-600">{{ number_format($updateCount) }}</p>
 </div>
 </div>
 </div>
 ...
 $errorRate = $totalRecords > 0 ? round((count($errorRows) / $totalRecords) * 100, 1) : 0;
 @endphp
 ...
 Véhicules Importés ({{ $successCount }} au total)
 </h3>
 </div>
 <div class="p-6">
 ...
 <p class="text-gray-600 mb-4">{{ $successCount }} véhicule(s) ont été importés avec succès dans la base de données.</p>
 <a href="{{ route('admin.vehicles.index') }}"
...
function downloadReport() {
 ...
 link.download = `import_vehicules_report_${new Date().toISOString().split('T')[0]}.csv`;
 link.click();
}
...
function generateCSVReport(data) {
 ...
 csv += `Importés avec succès,${data.successful_imports || 0},${(((data.successful_imports || 0) / total) * 100).toFixed(1)}%\n`;
 csv += `Mis à jour,${data.updated_existing || 0},${(((data.updated_existing || 0) / total) * 100).toFixed(1)}%\n`;
 csv += `Échecs,${data.failed_imports || 0},${(((data.failed_imports || 0) / total) * 100).toFixed(1)}%\n`;
 }

 if (data.errors && data.errors.length > 0) {
 csv += '\n\nLigne,Erreur,Données\n';
 data.errors.forEach(error => {
 csv += `${error.row || error.line || 'N/A'},"${error.error || 'Erreur inconnue'}","${JSON.stringify(error.data || {}).replace(/"/g, '""')}"\n`;
 });
 }
