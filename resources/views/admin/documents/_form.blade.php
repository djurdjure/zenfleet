{{--
    ...
    - $categories: An associative array of document categories (id => name).
...
<div x-data="documentForm({
    ...
    initialCategoryId: '{{ old('document_category_id', $document->document_category_id ?? '') }}',
    initialExtraMetadata: {{ json_encode(old('extra_metadata', $document->extra_metadata ?? [])) }},
    ...
    <form :action="actionUrl" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div>
                    ...
                    <select id="linked_vehicles" name="linked_vehicles[]" multiple>
                        @foreach($vehicles as $vehicle)
                            <option value="{{ $vehicle->id }}" @selected(in_array($vehicle->id, old('linked_vehicles', $document->vehicles->pluck('id')->toArray() ?? [])))>
                ...
                <div>
                    ...
                    <select id="linked_drivers" name="linked_drivers[]" multiple>
                        @foreach($drivers as $driver)
                            <option value="{{ $driver->id }}" @selected(in_array($driver->id, old('linked_drivers', $document->users->pluck('id')->toArray() ?? [])))>
                ...
                <div>
                    ...
                    <select id="linked_suppliers" name="linked_suppliers[]" multiple>
                        @foreach($suppliers as $supplier)
                            <option value="{{ $supplier->id }}" @selected(in_array($supplier->id, old('linked_suppliers', $document->suppliers->pluck('id')->toArray() ?? [])))>
                                {{ $supplier->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="p-6 border border-gray-200 rounded-lg">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <x-date-picker id="issue_date" name="issue_date" label="Date d'émission" :value="old('issue_date', $document->issue_date ?? null)" />
                    <x-input-error :messages="$errors->get('issue_date')" class="mt-2" />
                </div>
                <div>
                    <x-date-picker id="expiry_date" name="expiry_date" label="Date d'expiration" :value="old('expiry_date', $document->expiry_date ?? null)" />
