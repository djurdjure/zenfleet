...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 -m-6 p-6">
 ...
 <span class="font-semibold text-blue-600">{{ $driver->first_name }} {{ $driver->last_name }}</span>
 ...
 @endif
 <div>
 <div class="text-sm font-semibold text-gray-900">{{ $driver->status->name ?? 'Statut non défini' }}</div>
 ...
 @include('admin.drivers.partials.step2-professional', ['driver' => $driver])
 </div>
