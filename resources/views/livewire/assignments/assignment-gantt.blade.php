...
<div class="space-y-6" x-data="assignmentGantt()" x-init="init()">
    ...
    @if($showQuickCreateModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" @click="closeQuickCreateModal()"></div>
        ...
        function assignmentGantt() {
            return {
                ...
                handleAssignmentClick(assignmentId, event) {
                    ...
                    @this.dispatch('edit-assignment', { id: assignmentId });
                },
            ...
            return {
                tooltipContent: `
                    <div class="bg-gray-900 text-white p-3 rounded-lg shadow-lg max-w-sm">
                        ...
                        ${tooltipData.reason ? `<div class="text-xs text-gray-300 mt-2">Motif: ${tooltipData.reason}</div>` : ''}
                        <div class="flex justify-between items-center mt-2">
                            <span class="text-xs px-2 py-1 rounded bg-gray-700">${tooltipData.status}</span>
                        </div>
                    </div>
                `
            };
    ...
    <style>
        ...
        .gantt-timeline {
            ...
            background-size: {{ $viewMode === 'day' ? '80px' : ($viewMode === 'week' ? '120px' : '40px') }} 100%;
        }
