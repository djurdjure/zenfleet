    ...
    <div class="rounded-lg shadow-lg border-l-4 p-4" :class="typeClasses()">
        <div class="flex items-start">
            <div class="flex-shrink-0" :class="iconClasses()">
                <template x-if="type === 'success'"><x-icon icon="heroicons:check-circle" class="h-6 w-6" /></template>
                <template x-if="type === 'error'"><x-icon icon="heroicons:x-circle" class="h-6 w-6" /></template>
                <template x-if="type === 'warning'"><x-icon icon="heroicons:exclamation-triangle" class="h-6 w-6" /></template>
                <template x-if="type === 'info'"><x-icon icon="heroicons:information-circle" class="h-6 w-6" /></template>
            </div>
            ...
            <div class="ml-4 flex-shrink-0 flex">
                <button @click="show = false" class="inline-flex rounded-md text-gray-400 hover:text-gray-500 focus:outline-none">
                    ...
                    <x-icon icon="heroicons:x-mark" class="h-5 w-5" />
                </button>
            </div>
        </div>
    </div>
